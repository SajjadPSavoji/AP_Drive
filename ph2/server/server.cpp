...
using namespace std;
void Server::login_with_session_id(std::string sid ,Drive* drive)
{
  vector<string> command_splited;
  make_login_command(session_IDs[sid].first ,session_IDs[sid].second ,command_splited);
  drive->login(command_splited);
}
string Server::session_gen()
{
    srand(time(NULL));
    stringstream ss;
    for(int i = 0;i < 36;i++)
...
bool Server::is_session_valid(std::string session_id)
{
  cerr<<"______________sessions_________"<<endl;
  map<string,pair<string,string> >::iterator it;
  for (it=session_IDs.begin(); it != session_IDs.end(); it++) {
    cerr<<it->first<<endl;
    if(it->first == session_id)
      return true;
  }
  return false;
  // it = session_IDs.find(session_id);
  // if (it != session_IDs.end())
  //   return true;
  // return false;
}
std::pair<std::string,std::string> Server::get_user_by_session_ID(std::string session_id)
{
  if (is_session_valid(session_id))
    return session_IDs[session_id];
  return pair<string ,string> ("","");
}
void Server::expire_session_id(std::string session_id)
{
  map<string,pair<string,string> >::iterator it;
  it = session_IDs.find(session_id);
  if (it != session_IDs.end())
...
void Server::add_session_id(std::string username, std::string pass , std::string SID)
{
  session_IDs[SID]=pair<string ,string> (username , pass);
}
