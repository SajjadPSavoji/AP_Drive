...
using namespace std;
void make_mk_dir_command(std::vector<std::string> &command_splited , std::string new_dir)
{
	command_splited.clear();
	command_splited.push_back(MAKE_DIRECTORY);
	command_splited.push_back(new_dir);
}
void make_adduser_command(vector<string> &command_splited ,string username ,string password)
{
	command_splited.clear();
	command_splited.push_back(ADD_USER);
	command_splited.push_back(username);
	command_splited.push_back(password);
}
void make_userlist_command(vector<string> &command_splited )
{
	command_splited.clear();
	command_splited.push_back(USERLIST);
}
void make_login_command(string username,string pass ,vector<string> &command_splited )
{
	command_splited.clear();
	command_splited.push_back(LOGIN);
	command_splited.push_back(username);
	command_splited.push_back(pass);
}
void make_logout_command(std::vector<std::string> &command_splited)
{
	command_splited.clear();
	command_splited.push_back(LOGOUT);
}
