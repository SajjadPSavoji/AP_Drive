...
void Drive::user_list_back(std::vector<std::string> &users_listed)
{
	users_listed.clear();
	for (size_t i = 0; i < _users.size(); i++) {
		users_listed.push_back(_users[i]->_user_name);
	}
}
void Drive::files_list_back(vector<string> &files_listed ,vector<string> &directory_listed)
{
	files_listed.clear();
	directory_listed.clear();
	for (size_t i = 0; i < _current_directory->_files.size(); i++) {
		files_listed.push_back(_current_directory->_files[i]->_name);
	}
	for (size_t i = 0; i < _current_directory->_sub_directories.size(); i++) {
		directory_listed.push_back(_current_directory->_sub_directories[i]->_name);
	}
}
